  ...
  <div>
    ...
    <!-- 卡片视图区域 -->
     <el-card>
        ...
        <!-- 搜索框区域-->
      <el-row :gutter="20"><!-- gutter表示间距 -->
        <el-col :span="9"><!-- span表示占宽度比例 -->
      ...
      <el-table :data="userlist" border stripe>
        ...
        <el-table-column label="状态">
         <template slot-scope="scope">
            <el-switch v-model="scope.row.mg_state" @change="userStateChanged(scope.row)"><!-- slot-scope="scope"相当于一行的数据 -->
            </el-switch>
         </template>
        </el-table-column>
...
export default{
  ...
  methods:{
    ...
    //监听添加用户对话框的关闭事件
    addDialogClosed(){
      this.$refs.addFormRef.resetFields();//先获取表单的引用，再调用resetFields方法
    ...
    addUser(){
      this.$refs.addFormRef.validate(async valid=>{
        //如果校验不通过，直接返回
        if(!valid) return
        //如果校验通过，可以发起添加用户的网络请求
        const {data:res}=await this.$http.post('users',this.addForm)//api文档中使用post方法和users路径
        if(res.meta.status!==201){//api文档中响应数据返回值为201
